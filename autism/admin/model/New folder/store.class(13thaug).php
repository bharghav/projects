...
/*=========================================Product updating End	=============================================*/
	...
	function getAllOrdersList($sortfield,$order,$start,$limit)	
	{
		global $callConfig;		
		//if($sortfield!="" && $order!="") 
		//$whr='GROUP BY transactionid';
		
		$order=$whr.' '.$sortfield.' '.$order;		
	 	$query="select DISTINCT transactionid,dateon,orderid,status,totalprice,id FROM tb_orders ".$whr." ORDER BY id DESC LIMIT ".$start.",".$limit.""; 
		return $callConfig->getAllRows($query);
	} 
	
	
	
	function getAllOrdersListdb($countd,$sortfield,$order,$start,$limit)	
	{
		global $callConfig;	
		...
		if($_GET['week']!='') 
		{ 
		$query="select DISTINCT transactionid,dateon,orderid,status,totalprice,cust_id,pro_name,quantygrams FROM tb_orders where dateon BETWEEN '".$minusweek."' AND '".$todaydate."'";
		} 
		else if($_GET['month']!='') 
		{
		$query="select DISTINCT transactionid,dateon,status,orderid,totalprice,cust_id,pro_name,quantygrams FROM tb_orders where dateon BETWEEN '".$month."' AND '".$todaydate."'";
		
		} 
		else if($_GET['process']=='In Progress') 
		{
		$query="select DISTINCT transactionid,dateon,status,orderid,totalprice,cust_id,pro_name,quantygrams FROM tb_orders where status='In Progress'";
		} 
		else if($_GET['process']=='Decline') 
		{
		$query="select DISTINCT transactionid,dateon,status,orderid,totalprice,cust_id,pro_name,quantygrams FROM tb_orders where status='Decline'";		
		} 
		else if($_GET['process']=='Newordered') 
		{
		$query="select DISTINCT transactionid,dateon,status,orderid,totalprice,cust_id,pro_name,quantygrams FROM tb_orders where status='Newordered'";
		//$query="select 'GROUP BY transactionid' FROM tb_orders where status='Newordered'";
		}
		//echo "ssyam";
		//$query="SELECT * FROM tb_orders ".$whr." ORDER BY id DESC LIMIT ".$start.",".$limit.""; 
		//echo $query;exit;
		
		return $callConfig->getAllRows($query);
	} 
	
	function getAllOrdersListdb1($countd,$sortfield,$order,$start,$limit)	
	{
		global $callConfig;	
	...
	function getAllOrdersListdbCount($countd,$sortfield,$order,$start,$limit)	
	{
		global $callConfig;	
		...
		if($_GET['week']!='') {
		 ...
		 $query="select DISTINCT transactionid,dateon,cust_id,pro_name,quantygrams FROM tb_orders where dateon BETWEEN '".$minusweek."' AND '".$todaydate."'";
		 
		} else if($_GET['month']!='') {
		
		$query="select DISTINCT transactionid,dateon,cust_id,pro_name,quantygrams FROM tb_orders where dateon BETWEEN '".$month."' AND '".$todaydate."'";
		
		} else if($_GET['process']=='In Progress') {
		
		$query="select DISTINCT transactionid,cust_id,pro_name,quantygrams FROM tb_orders where status='In Progress'";
		
		} else if($_GET['process']=='Decline') {
		
		$query="select DISTINCT transactionid,cust_id,pro_name,quantygrams FROM tb_orders where status='Decline'";		
		
		} else if($_GET['process']=='Newordered') {
		
		$query="select DISTINCT transactionid,cust_id,pro_name,quantygrams FROM tb_orders where status='Newordered'";
	...
	function getAllOrdersListountshowdb($sortfield,$order,$start,$limit)	
	{
		global $callConfig;		
		if($sortfield!="" && $order!="") 
		...
		$order=$whr.' '.$sortfield.' '.$order;	
		$whr='GROUP BY transactionid';	
	$query="SELECT COUNT(DISTINCT transactionid) FROM tb_orders"; 
		return $callConfig->getCount($query);
	} 
	...
	function getorderprice($id)	
	{
		...
		$whr='transactionid="'.$id.'"';
		$order=$whr.' '.$sortfield.' '.$order;		
		 $query="SELECT * FROM tb_orders where transactionid='".$id."' ORDER BY id DESC LIMIT ".$start.",".$limit.""; 
		return $callConfig->getAllRows($query);
	} 
	
	
	 function getcountdata($id,$sortfield,$order,$start,$limit)

  {
	...
	if($sortfield!="" && $order!="") $order=$sortfield.' '.$order;
	 ...
	 $query=$callConfig->selectQuery(TPREFIX.TBL_ORDERS,'*','transactionid="'.$id.'"',$order,$start,$limit);

	return $callConfig->getAllRows($query);

  } 
	
	function getAllOrdersListstatus($status,$sortfield,$order,$start,$limit)	
	{
		global $callConfig;		
		/*if($sortfield!="" && $order!="") 
		$whr='GROUP BY txn_no';
		...
		$query="select DISTINCT transactionid,dateon,orderid,status,totalprice,id FROM tb_orders where status='".$status."' ORDER BY id DESC LIMIT ".$start.",".$limit."";
		return $callConfig->getAllRows($query);
	} 
	
	
	function getAllOrdersListstatuscount($status,$sortfield,$order,$start,$limit)	
	{
		
		global $callConfig;		
		
		$query="select DISTINCT transactionid,dateon,orderid,status,totalprice FROM tb_orders where status='".$status."' ORDER BY id DESC ";
		return $callConfig->getCount($query);
	} 
	
	
	
	function getAllOrderssearchList($post,$sortfield,$order,$start,$limit)	
	{
		global $callConfig;		
		if($sortfield!="" && $order!="") 		
		$fromdateArray=explode('/',$_POST['from']); 
		
		$fromdate = $fromdateArray[2].'-'.$fromdateArray[0].'-'.$fromdateArray[1];
		
		$todateArray=explode('/',$_POST['to']); 
		$todate = $todateArray[2].'-'.$todateArray[0].'-'.$todateArray[1];
		
		//print_r($fromdate); exit;
		
		if($_POST['orderfromnumber']!='' && $_POST['ordertonumber']=='' && $_POST['from']=='' && $_POST['to']=='') {
		
		$whr="orderid='".$_POST['orderfromnumber']."'";
		
		} else if($_POST['orderfromnumber']!='' && $_POST['ordertonumber']!='' && $_POST['from']=='' && $_POST['to']=='') {
		
		 $whr="orderid BETWEEN '".$_POST['orderfromnumber']."' AND '".$_POST['ordertonumber']."'";
		 
		} else if($_POST['from']!='' && $_POST['to']=='' && $_POST['orderfromnumber']=='' && $_POST['ordertonumber']=='') {
		
		 $whr="dateon='".$fromdate."'";
		
		} else if($_POST['from']!='' && $_POST['to']!='' && $_POST['orderfromnumber']=='' && $_POST['ordertonumber']=='') {
		
		$whr="dateon BETWEEN '".$fromdate."' AND '".$todate."'";
		
		} else if($_POST['orderfromnumber']!='' && $_POST['from']!='' && $_POST['to']=='' && $_POST['ordertonumber']=='') {
		
		$whr="orderid='".$_POST['orderfromnumber']."' and dateon='".$fromdate."'";
		
		} else if($_POST['orderfromnumber']!='' && $_POST['ordertonumber']!='' && $_POST['from']!='' && $_POST['to']=='') {
		
		$whr="orderid BETWEEN '".$_POST['orderfromnumber']."' AND '".$_POST['ordertonumber']."' and dateon='".$fromdate."'";
		
		} else  if($_POST['orderfromnumber']!='' && $_POST['ordertonumber']!='' && $_POST['from']!='' && $_POST['to']!='') {
		
		$whr="orderid BETWEEN '".$_POST['orderfromnumber']."' AND '".$_POST['ordertonumber']."' and dateon BETWEEN '".$fromdate."' AND '".$todate."'"; 
		
		}
			
	    $query="SELECT DISTINCT transactionid,dateon,orderid,status,totalprice FROM tb_orders where ".$whr." ORDER BY id DESC LIMIT ".$start.",".$limit.""; 
		//exit;
		return $callConfig->getAllRows($query);
	} 
	
	
	function getAllOrderssearchListCount($status,$sortfield,$order,$start,$limit)	
	{
		global $callConfig;		
		if($sortfield!="" && $order!="") 
		$whr='GROUP BY txn_no';
		$order=$whr.' '.$sortfield.' '.$order;		
		$query="SELECT DISTINCT transactionid,dateon,orderid,status,totalprice FROM tb_orders where status='".$status."' ORDER BY id DESC LIMIT ".$start.",".$limit."";
		return $callConfig->getCount($query);
	} 


  function getAllOrdersListCount($sortfield,$order,$start,$limit)
  {
	global $callConfig;
	//$query=$callConfig->selectQuery(TPREFIX.TBL_ORDERS,'id','','','','');
	$order=$whr.' '.$sortfield.' '.$order;		
	$query="select DISTINCT transactionid,dateon,orderid,status,totalprice FROM tb_orders ".$whr." ORDER BY id DESC LIMIT ".$start.",".$limit.""; 
  ...
  function OrderStatusChanging($get){
	...
	$res=$callConfig->deleteRecord(TPREFIX.TBL_ORDERS,'id',$id);

	if($res==1)

	{

	    $callConfig->deleteRecord(TPREFIX.TBL_ORDERS,'id',$id);
...
// end store coupons //
...

  {
	...
	global $callConfig;
	 ...
	 $query=$callConfig->selectQuery(TPREFIX.TBL_ORDERS,'*','transactionid="'.$id.'"','','','');
	return $callConfig->getAllRows($query);
	}


	function getsingleordertransaction($id) {
	global $callConfig;
	...
	function getorderadress($id) {
	global $callConfig;
	
	 $query=$callConfig->selectQuery(TPREFIX.TBL_CHECKOUT,'*','tempid="'.$id.'"','','','');
	return $callConfig->getRow($query);
	}
	
	
	function getcardaddress($id) {	
	global $callConfig;
	
	 $query=$callConfig->selectQuery(TPREFIX.TBL_CARDUSERS,'*','temprand="'.$id.'"','','','');
	return $callConfig->getRow($query);
	}
